...
#define PUT(sym)    std::make_pair(" #sym ", &p_sym)
...
read_program(std::istream& fin) {
...
#ifndef QASL_GRAMMAR_FILE
    exit_macro_does_not_exist("QASL_GRAMMAR_FILE");
#endif
    // Check that both the lexer file and grammar file both exist.
    test_file_exists(QASL_LEXER_FILE, "QASL_LEXER_FILE");
    test_file_exists(QASL_GRAMMAR_FILE, "QASL_GRAMMAR_FILE");
    ...
    LLParser qasl_parser(QASL_GRAMMAR_FILE);
